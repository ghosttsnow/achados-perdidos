...
import { Search, Plus, Shield, X, ArrowRight, LogOut } from 'lucide-react'
              ...
              {user && user.id !== 'guest' && (
                <div className="relative" ref={menuRef}>
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-green-50 border border-green-100 text-[#16a34a] text-sm font-medium hover:bg-green-100 transition-all duration-200"
                  >
                  ...
                  {showUserMenu && (
                    <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-xl shadow-slate-200/50 border border-slate-100 p-1.5 animate-dropdown-in z-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="relative text-center mb-12 sm:mb-16 py-8">
          ...
          <div className="absolute inset-0 -z-10 overflow-hidden">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-green-400/5 rounded-full blur-3xl" />
          </div>
          ...
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tight mb-4 animate-fade-in-up delay-75">
            Encontre seus{' '}
          ...
          <p className="text-slate-500 text-lg sm:text-xl max-w-lg mx-auto mb-10 animate-fade-in-up delay-150">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 animate-fade-in-up delay-200">
            ...
            <button
              ...
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-xl bg-white text-slate-700 font-semibold text-sm border border-slate-200 hover:bg-slate-50 hover:border-slate-300 hover:shadow-md active:scale-[0.97] transition-all duration-200"
            >
        ...
        <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-10 max-w-2xl mx-auto">
          {stats.map((stat, idx) => (
            <div
              key={stat.label}
              className={`${stat.bg} ${stat.border} border rounded-xl p-4 sm:p-5 text-center transition-all duration-200 hover:shadow-md cursor-default animate-fade-in-up`}
        ...
        <div className="max-w-2xl mx-auto mb-8">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-12 py-3.5 rounded-xl border border-slate-200 bg-white text-slate-900 placeholder-slate-400 focus:border-[#16a34a] focus:ring-2 focus:ring-green-50 focus:outline-none transition-all duration-200"
            />
            {search && (
              <button
                onClick={() => setSearch('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-all duration-150"
              >
